...
// 二元算术运算指令[iABC模式]
// 对两个寄存器或常量值进行运算(索引由操作数B和C指定)，将结果放入另一个寄存器(A指定)
// R(A) := RK(B) op RK(C)   // RK 寄存器或者常量值
...
// 一元算术运算指令[iABC模式]
// 对操作数B指定的寄存器中的值进行运算，然后把结果放入A指定的寄存器中， 未使用C
// R(A) := op R(B)
...
// CONCAT指令[iABC模式]
// 将连续n个寄存器(起止索引分别由B和C指定)里的值拼接，将结果放入另一个寄存器(A指定)
// R(A) := R(B)...R(C)
...
// 比较指令[iABC模式]
// 比较寄存器或常量表里的两个值(索引分别由B和C指定)，如果比较结果与A不匹配，则跳过下一条指令
...
// TESTSET指令[iABC模式]
// 判断寄存器(B指定)中的值转换为布尔值后是否与操作数C表示的布尔值一致
// 如果一致，则将B中的值复制到A，否则跳过下一条指令
...
// TEST指令[iABC模式]
// 判断寄存器(A指定)的值转换为布尔值后 是否与操作数C表示的布尔值一致
// 如果不一致 则跳过下一条指令
